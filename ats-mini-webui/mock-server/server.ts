    ...
    if (req.method === 'GET') {
      ...
      if (req.url?.endsWith('/tune')) {
        ...
        const newMemory = {
          id: memoryIdx,
          freq: status.freq,
          modeIdx: status.modeIdx,
          bandIdx: status.bandIdx,
          name: (status.rds?.stationName ?? '').substring(0, 9)
        }
        const savedMemory = memory.find(m => m.id === memoryIdx)
        if (!savedMemory) {
          memory.push(newMemory);
        } else {
          Object.assign(savedMemory, newMemory);
        }
        res.end(JSON.stringify(memory));
        ...
        req.on('end', () => {
          try {
            ...
            const savedMemory = memory.find(m => m.id === updatedMemory.id)
            if (!savedMemory) {
              memory.push(updatedMemory);
            } else {
              Object.assign(savedMemory, updatedMemory);
            }
            res.end(JSON.stringify(memory));
          } catch (error) {
            console.error(error)
      ...
      const savedMemoryIdx = memory.findIndex(m => m.id === memoryIdx)
      memory.splice(savedMemoryIdx, 1);
      res.end(JSON.stringify(memory));
    }
