...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  populateSelect('bands', statusOptions.bands.map(b => ({
    value: b.id.toString(),
    label: `${b.bandName} | ${bandModeDisplay(b.bandMode)} |  ${formatFrequency(b.minimumFreq, b.bandMode === 0 ? "FM" : undefined, true)} - ${formatFrequency(b.maximumFreq, b.bandMode === 0 ? "FM" : undefined, true)}`
  })));
